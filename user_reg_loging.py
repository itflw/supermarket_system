...
def check_user_name(user_name):
    ...
    # conn = pymysql.connect("192.168.9.108", "flw", "flw123", "mydb") 
    try:
...
def commit_user(user_name, user_passwd, user_phone, user_email):
    ...
    try:
        ...
        # sql = "insert into user (uname, passwd, phone, email) values (\"%s\", \"%s\", \"%s\", \"%s\")" %(user_name, user_passwd, user_phone, user_email)
            # sql = "insert into user (uname, passwd, phone, email) values (%s, %s, %s, %s)" %(user_name, user_passwd, user_phone, user_email)
            cur.execute("insert into user (uname, passwd, phone, email) values (%s, password(%s), %s, %s)", (user_name, user_passwd, user_phone, user_email))
            r = cur.rowcount
            conn.commit()
    finally:
        conn.close()
...
def reg_main():
    ...
    # 将用户名，密码提交到数据库，注册成功
    if commit_user(user_name, user_passwd, user_phone, user_email):
        print("注册成功")
    else:
        print("注册失败")
